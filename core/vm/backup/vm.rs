...
impl VM {
    ...
    fn handler_call1add(vm: &mut VM, insn: u32) -> ControlFlow {
        ...
        if let CallAction::Returned(result) =
            vm.dispatch_call_value(callee, vm.frame.regs[0], &[arg])
        {
            vm.frame.regs[ACC] = vm.add_values(vm.frame.regs[ACC], result);
        }
        ControlFlow::Continue
    }
    ...
    fn handler_call2add(vm: &mut VM, insn: u32) -> ControlFlow {
        ...
        if let CallAction::Returned(result) =
            vm.dispatch_call_value(callee, vm.frame.regs[0], &[arg1, arg2])
        {
            vm.frame.regs[ACC] = vm.add_values(vm.frame.regs[ACC], result);
        }
        ControlFlow::Continue
        ...
        while self.pc < self.bytecode.len() {
            ...
            match opcode {
                ...
                Opcode::Call1Add => {
                    ...
                    match self.dispatch_call_value(callee, self.frame.regs[0], &[arg]) {
                        CallAction::Returned(result) => {
                            self.frame.regs[ACC] = self.add_values(self.frame.regs[ACC], result);
                        }
                        CallAction::EnteredFrame => continue,
                    }
                }
